...
class JWTCompanyMiddleware:
    ...
    def __call__(self, request):
        # Allow test header injection in DEBUG mode or if specifically allowed
        # (Though in prod, Kong usually handles this, we do it here for double safety or direct access)
        header_uuid = request.headers.get("X-Company-UUID")
        if header_uuid:
             # Basic trust for internal propagation if needed, mostly for dev
             request.company_uuid = header_uuid
             request.user_claims = {"company_uuid": header_uuid, "permissions": []} # No perms via header usually
